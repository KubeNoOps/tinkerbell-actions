...
do_install() {
	echo "# Executing containerd install script"
	...
	case "$lsb_dist" in
		ubuntu|debian|raspbian)
			...
			$sh_c "DEBIAN_FRONTEND=noninteractive apt-get install -y -qq containerd.io >/dev/null"
		...
		centos|fedora|rhel)
			...
			pkg_version=""
			$sh_c "$pkg_manager install -y -q containerd.io"
		...
		sles)
			...
			$sh_c "zypper -q install -y containerd.io"
